...
class TestQueueExecutor : Executor {
    ...
    override fun executeDelayed(delay: Duration, block: () -> Unit): Executor.Future {
        ...
        dispatch_after(
            dispatch_time(DISPATCH_TIME_NOW, delay.milliseconds() * 1000),
    ...
    override fun schedule(updateRate: Duration, block: () -> Unit): Executor.Future {
        ...
        val timer = NSTimer(NSDate(), (updateRate.milliseconds().toDouble() * 1000.0), true) {
            if (it?.valid == true) {
